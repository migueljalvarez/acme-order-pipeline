// src/orders-events/order-event.strategy.service.ts
...
import {
  OrderCreatedItem,
  OrderEventInterface,
} from "./interfaces/orders-events.interface";
...
export class OrderEventStrategyService {
  ...
  private async handleOrderCreated(event: OrderEventInterface) {
    const existing = await this.orderEventService.validateOrderEvents(
      event.orderId
    );
    ...
    const orderEventCreated = await this.orderEventService.saveOrderEvent(
      orderEvents
    );
    this.logger.log(
      this.context,
  ...
  public async execute(event: OrderEventInterface) {
    const handlers: Record<
      number,
      (event: OrderEventInterface) => Promise<void>
    > = {
